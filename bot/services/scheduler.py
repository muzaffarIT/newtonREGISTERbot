...
from bot.services.google_sheets import sheets_service, is_student_cancelled, student_cancel_date

logger = logging.getLogger(__name__)

async def build_daily_report_text() -> str:
    """Формирует текст ежедневного отчёта (без отправки)."""
    ...
    for t in settings.ALL_BRANCHES:
        # Один упавший филиал не должен ронять весь отчёт
        try:
            groups = await sheets_service.get_groups_status(t)
        except Exception as e:
            logger.error(f"Failed to load branch {t}: {e}")
            continue
        if not groups:
            continue
    ...
    # Активность по студентам за сегодня
    students_today = []
    cancelled_today = 0
    ...
    for s in all_students:
        if len(s) == 0 or not s[0]:
            continue
        enroll_date = s[0][:10]
        # Отменено сегодня — по дате отмены (независимо от того, когда записан)
        if student_cancel_date(s) == iso_date:
            cancelled_today += 1
        # Записано сегодня и до сих пор активно
        if enroll_date == iso_date and not is_student_cancelled(s):
...
async def send_daily_report(bot: Bot):
    ...
    try:
        ...
        report_text = await build_daily_report_text()
